...
To examine the data, we decided to split the monitors into four quadrants according to the latitude and longitude lines that bisect the U.S. and compare CMAQ levels across these quadrants. We found that the northwestern quadrant had much lower CMAQ levels than the other three quadrants.
...
In using a random forest to find the best model for predicting air quality from our data: - Multiple decision trees are created - Each tree is built using a random subset of the training data with replacement - The randomness helps to keep the algorithm from overfitting the data - The mean of the predictions from each of the trees is used in the final output
